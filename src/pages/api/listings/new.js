...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // Check for duplicate listing by propertyDescription
const existingListing = await prisma.listing.findFirst({
  where: {
    propertyDescription: safeDescription.trim(),
  },
});

if (existingListing) {
  return res.status(409).json({ message: 'Duplicate listing detected: another agent has posted a similar property description.' });
}
